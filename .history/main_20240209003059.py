...
@app.post("/todos/update_todo/{id}", tags=['TODOS'])
async def edit_todo( new_item: Item, id: models.Items.id, db: Session = Depends(get_db)):
    item_to_edit = db.query(models.Items).filter(models.Items.id == id).one_or_none()
    if item_to_edit is None:
            return {"error":"....."}
    print(new_item.model_dump())
    for k, v in new_item.model_dump(exclude_unset=True).items():
